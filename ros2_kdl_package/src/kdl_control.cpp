...
//  CALCOLO DELLA Y
    ...
    y << Jpinv*(x_dotdot_d - robot_->getEEJacDotqDot() + Kd*x_dot_tilde + Kp*x_tilde);
     ...
     //  std::cout <<"robot_->getEEJacDotqDot()<"<<robot_->getEEJacDotqDot()<<std::endl;

     /*  std::cout <<std::endl<<"x_tilde                      ";
         for (int k=0; k<6;k++)
         std::cout << x_tilde(k)<<"  ";
                    

         std::cout <<std::endl<<"x_dot_tilde                      ";
         for (int k=0; k<6;k++)
     std::cout << x_dot_tilde(k)<<"  ";
                        
     std::cout <<std::endl<<"x_dotdot_d                      ";
     for (int k=0; k<6;k++)
     std::cout << x_dotdot_d(k)<<"  ";            

     std::cout <<std::endl<<"y                      ";
     for (int k=0; k<6;k++)
     std::cout << y(k)<<"  ";

     std::cout<<std::endl << Kp << std::endl;

     std::cout <<std::endl<< Kd << std::endl;*/
